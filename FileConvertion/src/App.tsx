  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch(
        `https://heictojpeg.onrender.com/upload-and-convert-to/${convertTo}`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        setIsLoading(false);
        setError(
          "Either you tried to convert an unsupported file, or the server is cooked."
        );
      ...
      if (blob.size === 0) {
        setError(
          "Error cus you probably tried to convert an unsupported file!"
        );
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="h-[20rem] border bg-gradient-to-bl from-gray-900 to-gray-400 flex items-center justify-center flex-col">
        <h2 className="text-white md:text-4xl text-3xl">
          Image File Converter
        </h2>
        <p className="text-neutral-400 text-2xl">
          Convert photos/images from your iphone to a format your computer will
          not complain about :D.
        </p>
      </div>

      <div className="flex flex-col items-center justify-center mt-7 p-7">
        <h2 className="text-red-500 text-2xl">
          If you're using the deployed version of this website, their is a very
          high chance the convertion will take a while because I&apos;m using
          the free tier for Render (I&apos;m broke I&apos;m sorry). They&apos;re
        ...
        <h2>Select file(s):</h2>
        <input
          className="border rounded-md h-[2rem]"
          type="file"
          multiple
      ...
      <section className="flex flex-col items-center justify-center p-2">
        <div className="flex flex-col items-center mt-7 border w-[50%] min-h-[20rem] p-2 rounded-3xl gap-2">
          ...
          <div className="flex items-center justify-between mt-2 gap-2"></div>
        </div>
      ...
      <section className="flex flex-col items-center justify-center mt-2 p-2 gap-7">
        ...
        <p>
          If you actually want to know how it works and how I built it,{" "}
          <span>
            <a
              className="underline text-blue-400 hover:text-blue-200"
              href="https://github.com/Skennymon/HEICtoJPEG.git/"
              target="_blank"
            >
